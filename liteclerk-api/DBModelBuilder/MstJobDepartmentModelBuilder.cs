    ...
    public class MstJobDepartmentModelBuilder
    {
        public static void CreateMstJobDepartmentModel(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DBSets.MstJobDepartmentDBset>(entity =>
            {
                ...
                entity.HasOne(f => f.MstUser_CreatedByUserId).WithMany(f => f.MstJobDepartments_CreatedByUserId).HasForeignKey(f => f.CreatedByUserId).OnDelete(DeleteBehavior.Restrict);
                ...
                entity.HasOne(f => f.MstUser_UpdatedByUserId).WithMany(f => f.MstJobDepartments_UpdatedByUserId).HasForeignKey(f => f.UpdatedByUserId).OnDelete(DeleteBehavior.Restrict);
