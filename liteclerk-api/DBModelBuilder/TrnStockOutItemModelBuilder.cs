    ...
    public class TrnStockOutItemModelBuilder
    {
        public static void CreateTrnStockOutItemModel(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DBSets.TrnStockOutItemDBSet>(entity =>
            {
                ...
                entity.HasOne(f => f.TrnStockOut_OTId).WithMany(f => f.TrnStockOutItems_OTId).HasForeignKey(f => f.OTId).OnDelete(DeleteBehavior.Cascade);
                ...
                entity.HasOne(f => f.MstArticleItemInventory_ItemInventoryId).WithMany(f => f.TrnStockOutItems_ItemInventoryId).HasForeignKey(f => f.ItemInventoryId).OnDelete(DeleteBehavior.Restrict);
