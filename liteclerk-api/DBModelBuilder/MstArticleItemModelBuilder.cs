    ...
    public class MstArticleItemModelBuilder
    {
        public static void CreateMstArticleItemModel(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DBSets.MstArticleItemDBSet>(entity =>
            {
                ...
                entity.HasOne(f => f.MstTax_RRVAT).WithMany(f => f.MstArticleItems_RRVAT).HasForeignKey(f => f.RRVATId).OnDelete(DeleteBehavior.Restrict);
                entity.Property(e => e.SIVATId).HasColumnName("SIVATId").HasColumnType("int").IsRequired();
                entity.HasOne(f => f.MstTax_SIVAT).WithMany(f => f.MstArticleItems_SIVAT).HasForeignKey(f => f.SIVATId).OnDelete(DeleteBehavior.Restrict);
                entity.Property(e => e.WTAXId).HasColumnName("WTAXId").HasColumnType("int").IsRequired();
                entity.HasOne(f => f.MstTax_WTAX).WithMany(f => f.MstArticleItems_WTAX).HasForeignKey(f => f.WTAXId).OnDelete(DeleteBehavior.Restrict);
