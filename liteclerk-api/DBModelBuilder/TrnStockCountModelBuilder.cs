    ...
    public class TrnStockCountModelBuilder
    {
        public static void CreateTrnStockCountModel(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DBSets.TrnStockCountDBSet>(entity =>
            {
                ...
                entity.Property(e => e.SCNumber).HasColumnName("SCNumber").HasColumnType("nvarchar(50)").HasMaxLength(50).IsRequired();
                entity.Property(e => e.SCDate).HasColumnName("SCDate").HasColumnType("datetime").IsRequired();
                ...
                entity.HasOne(f => f.MstUser_PreparedByUserId).WithMany(f => f.TrnStockCounts_PreparedByUserId).HasForeignKey(f => f.PreparedByUserId).OnDelete(DeleteBehavior.Restrict);

                entity.Property(e => e.CheckedByUserId).HasColumnName("CheckedByUserId").HasColumnType("int").IsRequired();
                entity.HasOne(f => f.MstUser_CheckedByUserId).WithMany(f => f.TrnStockCounts_CheckedByUserId).HasForeignKey(f => f.CheckedByUserId).OnDelete(DeleteBehavior.Restrict);

                entity.Property(e => e.ApprovedByUserId).HasColumnName("ApprovedByUserId").HasColumnType("int").IsRequired();
                entity.HasOne(f => f.MstUser_ApprovedByUserId).WithMany(f => f.TrnStockCounts_ApprovedByUserId).HasForeignKey(f => f.ApprovedByUserId).OnDelete(DeleteBehavior.Restrict);
