    ...
    public class MstPayTypeModelBuilder
    {
        public static void CreateMstPayTypeModel(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DBSets.MstPayTypeDBSet>(entity =>
            {
                ...
                entity.HasOne(f => f.MstAccount_Account).WithMany(f => f.MstPayTypes_Account).HasForeignKey(f => f.AccountId).OnDelete(DeleteBehavior.Restrict);
                entity.Property(e => e.CreatedByUserId).HasColumnName("CreatedByUserId").HasColumnType("int").IsRequired();
                entity.HasOne(f => f.MstUser_CreatedByUser).WithMany(f => f.MstPayTypes_CreatedByUser).HasForeignKey(f => f.CreatedByUserId).OnDelete(DeleteBehavior.Restrict);
                ...
                entity.HasOne(f => f.MstUser_UpdatedByUser).WithMany(f => f.MstPayTypes_UpdatedByUser).HasForeignKey(f => f.UpdatedByUserId).OnDelete(DeleteBehavior.Restrict);
