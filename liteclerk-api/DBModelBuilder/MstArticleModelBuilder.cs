    ...
    public class MstArticleModelBuilder
    {
        public static void CreateMstArticleModel(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DBSets.MstArticleDBSet>(entity =>
            {
                ...
                entity.HasOne(f => f.MstArticleType_ArticleType).WithMany(f => f.MstArticles_ArticleType).HasForeignKey(f => f.ArticleTypeId).OnDelete(DeleteBehavior.Restrict);
                ...
                entity.HasOne(f => f.MstUser_CreatedByUser).WithMany(f => f.MstArticles_CreatedByUser).HasForeignKey(f => f.CreatedByUserId).OnDelete(DeleteBehavior.Restrict);
                ...
                entity.HasOne(f => f.MstUser_UpdatedByUser).WithMany(f => f.MstArticles_UpdatedByUser).HasForeignKey(f => f.UpdatedByUserId).OnDelete(DeleteBehavior.Restrict);
