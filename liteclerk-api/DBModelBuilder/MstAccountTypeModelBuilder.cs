    ...
    public class MstAccountTypeModelBuilder
    {
        public static void CreateMstAccountTypeModel(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DBSets.MstAccountTypeDBSet>(entity =>
            {
                ...
                entity.HasOne(f => f.AccountCategory).WithMany(f => f.MstAccountTypes_AccountCategory).HasForeignKey(f => f.AccountCategoryId).OnDelete(DeleteBehavior.Restrict);
                entity.Property(e => e.CreatedByUserId).HasColumnName("CreatedByUserId").HasColumnType("int").IsRequired();
                entity.HasOne(f => f.CreatedByUser).WithMany(f => f.MstAccountTypes_CreatedByUser).HasForeignKey(f => f.CreatedByUserId).OnDelete(DeleteBehavior.Restrict);
                ...
                entity.HasOne(f => f.UpdatedByUser).WithMany(f => f.MstAccountTypes_UpdatedByUser).HasForeignKey(f => f.UpdatedByUserId).OnDelete(DeleteBehavior.Restrict);
