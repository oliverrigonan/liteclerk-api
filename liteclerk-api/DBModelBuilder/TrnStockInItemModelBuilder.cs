    ...
    public class TrnStockInItemModelBuilder
    {
        public static void CreateTrnStockInItemModel(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DBSets.TrnStockInItemDBSet>(entity =>
            {
                ...
                entity.HasOne(f => f.TrnStockIn_INId).WithMany(f => f.TrnStockInItems_INId).HasForeignKey(f => f.INId).OnDelete(DeleteBehavior.Cascade);

                entity.Property(e => e.JOId).HasColumnName("JOId").HasColumnType("int");
                entity.HasOne(f => f.TrnJobOrder_JOId).WithMany(f => f.TrnStockInItems_JOId).HasForeignKey(f => f.JOId);
