    ...
    public class MstAccountModelBuilder
    {
        public static void CreateMstAccountModel(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DBSets.MstAccountDBSet>(entity =>
            {
                ...
                entity.HasOne(f => f.AccountType).WithMany(f => f.MstAccounts_AccountType).HasForeignKey(f => f.AccountTypeId).OnDelete(DeleteBehavior.Restrict);
                entity.Property(e => e.AccountCashFlowId).HasColumnName("AccountCashFlowId").HasColumnType("int").IsRequired();
                entity.HasOne(f => f.AccountCashFlow).WithMany(f => f.MstAccounts_AccountCashFlow).HasForeignKey(f => f.AccountCashFlowId).OnDelete(DeleteBehavior.Restrict);
                entity.Property(e => e.CreatedByUserId).HasColumnName("CreatedByUserId").HasColumnType("int").IsRequired();
                entity.HasOne(f => f.CreatedByUser).WithMany(f => f.MstAccounts_CreatedByUser).HasForeignKey(f => f.CreatedByUserId).OnDelete(DeleteBehavior.Restrict);
                ...
                entity.HasOne(f => f.UpdatedByUser).WithMany(f => f.MstAccounts_UpdatedByUser).HasForeignKey(f => f.UpdatedByUserId).OnDelete(DeleteBehavior.Restrict);
