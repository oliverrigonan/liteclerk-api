    ...
    public class MstUnitModelBuilder
    {
        public static void CreateMstUnitModel(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DBSets.MstUnitDBSet>(entity =>
            {
                ...
                entity.HasOne(f => f.CreatedByUser).WithMany(f => f.MstUnits_CreatedByUser).HasForeignKey(f => f.CreatedByUserId).OnDelete(DeleteBehavior.Restrict);
                ...
                entity.HasOne(f => f.UpdatedByUser).WithMany(f => f.MstUnits_UpdatedByUser).HasForeignKey(f => f.UpdatedByUserId).OnDelete(DeleteBehavior.Restrict);
