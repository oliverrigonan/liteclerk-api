    ...
    public class MstArticleCustomerModelBuilder
    {
        public static void CreateMstArticleCustomerModel(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DBSets.MstArticleCustomerDBSet>(entity =>
            {
                ...
                entity.HasOne(f => f.Article).WithMany(f => f.MstArticleCustomers_Article).HasForeignKey(f => f.ArticleId).OnDelete(DeleteBehavior.Cascade);
                ...
                entity.HasOne(f => f.ReceivableAccount).WithMany(f => f.MstArticleCustomers_ReceivableAccount).HasForeignKey(f => f.ReceivableAccountId).OnDelete(DeleteBehavior.Restrict);
                entity.Property(e => e.TermId).HasColumnName("TermId").HasColumnType("int");
                entity.HasOne(f => f.Term).WithMany(f => f.MstArticleCustomers_Term).HasForeignKey(f => f.TermId).OnDelete(DeleteBehavior.Restrict);
