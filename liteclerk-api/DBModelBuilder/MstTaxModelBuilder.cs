    ...
    public class MstTaxModelBuilder
    {
        public static void CreateMstTaxModel(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<DBSets.MstTaxDBSet>(entity =>
            {
                ...
                entity.HasOne(f => f.MstUser_CreatedByUser).WithMany(f => f.MstTaxes_CreatedByUser).HasForeignKey(f => f.CreatedByUserId).OnDelete(DeleteBehavior.Restrict);
                ...
                entity.HasOne(f => f.MstUser_UpdatedByUser).WithMany(f => f.MstTaxes_UpdatedByUser).HasForeignKey(f => f.UpdatedByUserId).OnDelete(DeleteBehavior.Restrict);
