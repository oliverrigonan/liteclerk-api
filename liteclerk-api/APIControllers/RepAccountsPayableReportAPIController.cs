...
namespace liteclerk_api.APIControllers
{
    ...
    [ApiController]
    public class RepAccountsPayableReportAPIController : ControllerBase
    {
        private readonly DBContext.LiteclerkDBContext _dbContext;

        public RepAccountsPayableReportAPIController(DBContext.LiteclerkDBContext dbContext)
        {
        ...
        public async Task<ActionResult> GetAccountsPayableReportList(String dateAsOf, Int32 companyId, Int32 branchId)
        {
            if (branchId == 0)
            {
                IEnumerable<DTO.RepAccountsPayableReportDTO> receivingReceipts = await (
                    ...
                    && d.BalanceAmount > 0
                    && d.IsLocked == true
                    select new DTO.RepAccountsPayableReportDTO
                    {
                        ...
                        DueDate = d.RRDate.AddDays(Convert.ToInt32(d.MstTerm_TermId.NumberOfDays)).ToShortDateString(),
                        BalanceAmount = d.BalanceAmount,
                        CurrentAmount = ComputeAge(0, Convert.ToDateTime(dateAsOf).Subtract(d.RRDate.AddDays(Convert.ToInt32(d.MstTerm_TermId.NumberOfDays))).Days, d.BalanceAmount),
                        Age30Amount = ComputeAge(1, Convert.ToDateTime(dateAsOf).Subtract(d.RRDate.AddDays(Convert.ToInt32(d.MstTerm_TermId.NumberOfDays))).Days, d.BalanceAmount),
                        Age60Amount = ComputeAge(2, Convert.ToDateTime(dateAsOf).Subtract(d.RRDate.AddDays(Convert.ToInt32(d.MstTerm_TermId.NumberOfDays))).Days, d.BalanceAmount),
                        Age90Amount = ComputeAge(3, Convert.ToDateTime(dateAsOf).Subtract(d.RRDate.AddDays(Convert.ToInt32(d.MstTerm_TermId.NumberOfDays))).Days, d.BalanceAmount),
                        Age120Amount = ComputeAge(4, Convert.ToDateTime(dateAsOf).Subtract(d.RRDate.AddDays(Convert.ToInt32(d.MstTerm_TermId.NumberOfDays))).Days, d.BalanceAmount)
                    }
                ).ToListAsync();
