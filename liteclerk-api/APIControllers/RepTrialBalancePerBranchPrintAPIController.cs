...
namespace liteclerk_api.APIControllers
{
    ...
    [ApiController]
    public class RepTrialBalancePerBranchPrintAPIController : ControllerBase
    {
        private readonly DBContext.LiteclerkDBContext _dbContext;

        public RepTrialBalancePerBranchPrintAPIController(DBContext.LiteclerkDBContext dbContext)
        {
            _dbContext = dbContext;
        }

        [HttpGet("print/{startDate}/{endDate}/{companyId}/{branchId}")]
        public async Task<ActionResult> PrintTrialBalance(String startDate, String endDate, Int32 companyId, Int32 branchId)
        {
            ...
            if (loginUser != null)
            {
                DBSets.MstUserFormDBSet loginUserForm = await (
                    ...
                    && d.SysForm_FormId.Form == "ReportTrialBalancePerBranch"
                    select d
                ).FirstOrDefaultAsync();
                ...
                if (loginUserForm != null)
                {
                    if (loginUserForm.CanPrint == true)
                    {
                        ...
                        tableHeader.AddCell(new PdfPCell(new Phrase("TRIAL BALANCE PER BRANCH", fontSegoeUI13Bold)) { Border = 0,PaddingBottom = 10f, PaddingTop = 8f, Colspan = 2, HorizontalAlignment = Element.ALIGN_CENTER });
